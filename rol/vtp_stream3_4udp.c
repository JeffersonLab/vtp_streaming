...
rocPrestart()
{
  ...
  for(i=0;i<NUM_VTP_CONNECTIONS;i++) {
    vtpStreamingEvioWriteControl(i,EV_PRESTART,rol->runNumber,rol->runType);
  }
...
rocEnd()
{
  ...
  unsigned long long nBytes, nWords=0;
  ...
  printf(" %d Streams sent %d Frames each\n",NUM_VTP_CONNECTIONS, nFrames);
  for(ii=0;ii<NUM_VTP_CONNECTIONS;ii++) {
    nBytes = vtpStreamingBytesSent(ii);
    printf("  Stream %d sent %llu bytes\n",ii, nBytes);
    nWords += nBytes;
  }
  printf("Total of %llu words\n",nWords);
