...
rocDownload()
{
  ...
  //const char *z7file="fe_vtp_z7_streamingv3_aug12.bin";        /* 10Gb links */
  const char *z7file="fe_vtp_z7_streamingv3_Jan11_23.bin";       /* fixed UDP checksum bug */
  ...
  //  const char *v7file="fe_vtp_v7_fadc_streamingv3_Aug22.bin";        /* 10 Gb links - fixed PP header issue*/
  // const char *v7file="fe_vtp_v7_fadc_streamingv3_sept14.bin";        /* Add EB frame counters */
  //const char *v7file="fe_vtp_v7_fadc_streamingv3_sept27.bin";        /* Fixed frame counter and 3 stream missing header bugs */
  const char *v7file="fe_vtp_v7_fadc_streamingv3_dec9.bin";        /* Fixed conjested data bug and replaced UDP frame counter in UDP header */
...
rocPrestart()
{
  ...
  vtpPayloadConfig(10,ppInfo,1,1,0,2);
  vtpPayloadConfig(12,ppInfo,1,1,0,2);
  vtpPayloadConfig(13,ppInfo,1,1,0,2);
  ppmask = vtpPayloadConfig(15,ppInfo,1,1,0,2);
  ...
  /* Get Stream connection info from file. Then Setup the VTP connection registers manually and connect */
  {
	...
	ipaddr[3] = ipaddr[3] + 1;
	mac[4]    =    mac[4] + 1;
	//emuport   = emuport   + 1;  // Do not bump the port number if you attach to the CODA EMU Aggregator
      }
      ...
      emuData[7] = inst+1; /* connection number should start at 1 */
      vtpStreamingTcpConnect(inst, (netMode+1), emuData, 8);
...
rocEnd()
{
  ...
  //  vtpStreamingEbReset();
  /* Try just disabling the Stream inputs instead - Doing a full reset also zeros frame counters */
  /*  stat = vtpStreamingEbDisable(VTP_STREB_PP_STREAM_EN);
  if(stat != OK)
    printf("Error in vtpStreamingEbDisable()\n");
  */
